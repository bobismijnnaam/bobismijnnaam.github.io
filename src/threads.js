        ...
        if ((!proc.isRunning() && !proc.errorFlag) || proc.isDead) {
            if (proc.topBlock instanceof BlockMorph) {
                ...
                if (count) {
                    ...
                    if (proc.highlightedCustomBlock) {
                        proc.highlightedCustomBlock.removeHighlight();
                    }
                }
            }
            if (proc.prompter) {
...
Process.prototype.doReport = function (block) {
    ...
    if (this.context.inputs.length == 0) {
        this.pushContext(block.inputs()[0], this.context.outerContext);
    } else {
        var report = this.context.inputs[0];
        ...
        this.context.outerContext.variables.addVar('result');
        this.context.outerContext.variables.setVar('result', report);
...
Process.prototype.evaluate = function (
    ...
    if (!outer.receiver) {
        x.receiver = context.receiver; // for custom blocks
    }
...
Process.prototype.evaluateCustomBlock = function () {
    ...
    if (parms.length > 0) {
        ...
        for (i = 0; i < context.inputs.length; i += 1) {
            ...
            // snapshot for \old variables
            let oldValue = (value instanceof List) ? value.copy() : value;
            outer.variables.addVar(Symbol.for('\old' + context.inputs[i]), oldValue);
    ...
    // Referencing the custom block for error reports (VSnap!)
    this.markParentCustomBlock(runnable.expression, 
        this.context.expression);
};

Process.prototype.markParentCustomBlock = function (exprList, pcb) {
    for (e of exprList) {
        e.parentCustomBlock = pcb;
        this.markParentCustomBlock(e.children, pcb)
    }
};
...
Process.prototype.doUntilWithInvariant = function (
    goalCondition,
    inv,
    body) {
    var i, pcb;
    for (i = 0; i < inv.contents.length; i++) {
        x = inv.contents[i];
        if (!x) {

            let diag = new DialogBoxMorph();
            this.stop();
            this.errorFlag = true;
            pcb = this.context.expression.parentCustomBlock;
            pcb.addErrorHighlight();
            var mssg = "";
            mssg = this.context.accumulator === null ?
                "Invariant failed at entrance" : "Invariant not maintained by loop";

            diag.inform(
                mssg,
                `Proposition #${i + 1} violated at invariant from your block ${pcb.blockSpec}`,
                this.blockReceiver().world(),
                // this.context.expression.children[8].children[i].fullImage()
                this.context.expression.fullImage()
            );
            this.highlightedCustomBlock = pcb;
            break;
        }
    }
...
Process.prototype.doWhileWithInvariant = function (
    goalCondition,
    inv,
    body) 
{
...
Process.prototype.reportNeq = function (a, b) {
    return !snapEquals(a, b);
};
...
Process.prototype.doVerify = function (conditions, elements, name) {
    ...
    for (i = 0; i < elements.length(); i++) {
        ...
        if (b != null && !b) {
            ...
            diag.inform(
                `Contract failed!`,
                `This ${name} has been violated in your block ${pcb.blockSpec}.`,
                this.blockReceiver().world(),
                conditions[i].fullImage());
...
Process.prototype.reportLessOrEqual = function (a, b) {
    var x = +a,
        y = +b;
    if (isNaN(x) || isNaN(y)) {
        x = a;
...
Process.prototype.reportGreaterOrEqual = function (a, b) {
    var x = +a,
        y = +b;
    if (isNaN(x) || isNaN(y)) {
        x = a;
...
Process.prototype.doAssert = function (predicates) {

    var i, b, pcb, expr, lastChild = null;
    for (i = 0; i < predicates.length(); i++) {
        b = predicates.contents[i];
        if (b != null && !b) {
            let diag = new DialogBoxMorph();
            this.stop();
            this.errorFlag = true;
            ...
            pcb = this.context.expression.parentCustomBlock;
            if (!pcb) {
                pcb = this.context.expression.parentThatIsA(BlockMorph)
                mssg = `Predicate #${i + 1} of this assertion has failed in your script for #${pcb.parentThatIsA(ScriptsMorph).scriptTarget().name}`
            } else {
                mssg = `Predicate #${i + 1} of this assertion has failed inside your block '${pcb.blockSpec}'`
            }
            pcb.addErrorHighlight();


            // we do not  want to print the children commands
            expr = this.context.expression;
            lastChild = expr.children[expr.children.length - 1]
            if (lastChild instanceof CommandBlockMorph) {
                expr.children.pop();
            }
...
Process.prototype.specialVariableResult = function () {
    return null;
};

Process.prototype.reportForall = function (upvar, start, end, predicate) {
    ...
    if (dta === null) {
        ...
        if (this.context.inputs.pop() === false) {
            this.returnValueToParentContext(
                false
            );
            return;
    ...
    // this.pushContext('doYield');
    this.pushContext();
    this.evaluate(predicate, dta.parms, true);
};

Process.prototype.reportExists = function (upvar, start, end, predicate) {
...
Process.prototype.reportResult = function () {
    this.returnValueToParentContext(this.context.outerContext.variables.getVar('result'));
};

Process.prototype.reportOld = function (input) {
    this.returnValueToParentContext(this.context.outerContext.variables.getVar(Symbol.for('\old' + input.expression.blockSpec)));
};

Process.prototype.doForWithInvariant = function (
    upvar,
    start,
    end,
    invar,
    script) {
    // perform a script for every integer step between start and end,
    // assigning the current iteration index to a variable with the
    // name specified in the "upvar" parameter, so it can be referenced
    // within the script.
    ...
    if (dta === null) { // first time entered
        this.assertType(start, 'number');
        this.assertType(end, 'number');
        dta = this.context.accumulator = {
            test: +start < +end ?
        ...
        // check invariant at entrance
        this.pushContext();
        this.evaluate(invar, dta.parms);
        return;
    } else {
        if (this.context.inputs.length <= 5) {
            // need to evaluate the invariant and 
            // return to this context with its value
            dta.looped = dta.looped + 1;
            vars.changeVar(upvar, dta.step);
            this.pushContext();
            this.evaluate(invar, dta.parms);
        } else {
            // Break if the invariant did not pass
            // otherwise do body.
            if (!this.context.inputs.pop()) {
                let diag = new DialogBoxMorph();
                let parent = this.context.expression.parentThatIsA(
                    CustomCommandBlockMorph,
                    CustomReporterBlockMorph);
                var mssg = "";
                mssg = dta.looped <= 0 ?
                    "Invariant failed at entrance" :
                    "Invariant not maintained by loop";
                diag.inform(
                    mssg,
                    `Invariant failed at ${parent}`,
                    this.blockReceiver().world(),
                    this.context.expression.fullImage());

                throw Error(`Invariant ${invar} failed!`);
            }
            this.pushContext('doYield');
            this.pushContext();
            this.evaluate(script, dta.parms, true);
